...
class STOMP:
    ...
    def generate_noisy_trajectories(self, R_inv, trajectory):
        ...
        epsilon = np.random.multivariate_normal(
            mean=np.zeros(self.N), cov=R_inv, size=(self.K, self.d)
        )  # K, d, N
        epsilon = self.stddev * epsilon.transpose(
            0, 2, 1
        )  # K, N, d scale with std per each joint
    ...
    def compute_probability_P(self, S):
        ...
        denominator = np.sum(
            numerator, axis=0, keepdims=True
        )  # sum over k trajectories
    ...
    def unupdate_trajectory(self, trajectory, delta_trajectory_smoothed):
        trajectory = (
            trajectory - delta_trajectory_smoothed
    ...
    def compute_trajectory_total_cost_Q(self, trajectory, R):
        ...
        trajectory_cost = (
            self.func_obstacle_cost_single(trajectory)
            + self.func_constraint_cost_single(trajectory)
            + self.func_torque_cost_single(trajectory)
            + self.compute_trajectory_distance_cost(trajectory)
        )
    ...
    def optimize(self, trajectory):
        ...
        for iters in range(self.num_iterations):
            noisy_trajectories, epsilon = self.generate_noisy_trajectories(
                R_inv_scaled, trajectory_opt
            )
            ...
            delta_trajectory_smoothed = self.smooth_noisy(
                M, delta_trajectory_noise
            )

            trajectory_opt = self.update_trajectory(
                trajectory_opt, delta_trajectory_smoothed
            )
            ...
            if (
                ...
                trajectory_opt = self.unupdate_trajectory(
                    trajectory_opt, delta_trajectory_smoothed
                )
...
if __name__ == "__main__":
    ...
    if True:
        ...
        for point in trajectory_opt:
            circle1 = plt.Circle(
                (point[0], point[1]), radius=0.2, color="r", fill=False, alpha=0.5
            )
        ...
        if True:
            ...
            for i in range(d):
                axs55[i].plot(
                    times, trajectory[..., i], "k--", label=f"Original {i+1}"
                )
                axs55[i].plot(
                    times, trajectory_opt[..., i], "r+", label=f"Optimal {i+1}"
                )
                axs55[i].plot(
                    times,
                    [-np.pi] * len(times),
                    "m",
                    label=f"Joint Min : {-np.pi:.3f}",
                )
                axs55[i].plot(
                    times,
                    [np.pi] * len(times),
                    "c",
                    label=f"Joint Max : {np.pi:.3f}",
                )
