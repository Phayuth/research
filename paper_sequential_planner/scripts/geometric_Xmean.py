...
def sum_euclid_dist_sq(points, centre, weights=None):
    ...
    dist = np.sum(
        np.sqrt(np.sum(np.power(weights * (points - centre), 2), axis=1))
    )
    return dist
...
def fit(data, kmax=20, kmin=2, weights=None):
    ...
    n_free_centres = kmax - len(
        clusters
    )  # number of new centroids that can be added
    ...
    while (len(clusters) < kmax) & (change == 1):
        ...
        for i in range(0, len(clusters)):
            ...
            if len(parent_cluster) > 1:
                # split cluster
                child_clusters, child_centres = splitCluster(
                    data, parent_cluster, parent_centre
                )
                ...
                # calculate BIC
                parent_bic = BIC(
                    [data[parent_cluster, :]], [parent_centre], weights
                )
                children_bic = BIC(
                    [data[child_cluster1, :], data[child_cluster2, :]],
                    child_centres,
                    weights,
                )
    ...
    if weights is None:
        ...
        kmeans = KMeans(n_clusters=len(clusters), random_state=0).fit(
            weights * data
        )
    labels = kmeans.labels_
    centres = kmeans.cluster_centers_
