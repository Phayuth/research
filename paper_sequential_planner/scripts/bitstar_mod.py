    ...
    def plot_visited_connect(V1, V2):
        ...
        for k in range(max(len1, len2)):
            if k < len1:
                if V1[k].parent:
                    plt.plot(
                        [V1[k].x, V1[k].parent.x], [V1[k].y, V1[k].parent.y], "-g"
                    )
            if k < len2:
                if V2[k].parent:
                    plt.plot(
                        [V2[k].x, V2[k].parent.x], [V2[k].y, V2[k].parent.y], "-g"
                    )
...
class BITStar:
                ...
                if (
                    ...
                    if self.g_T[vm] + actual_cost < self.g_T[xm]:
                        ...
                        for v, x in self.Tree.QE:
                            if (
                                x == xm
                                and self.g_T[v] + self.calc_dist(v, xm)
                                >= self.g_T[xm]
                            ):
                                set_delete.add((v, x))
    ...
    def Sample(self, m, cMax, cMin, xCenter, C):
        if cMax < np.inf:
            ...
            return self.SampleFreeSpaceEllipsoid(m, cMax, cMin, xCenter, C)
    ...
    def SampleEllipsoid(self, m, cMax, cMin, xCenter, C):
        ...
        while ind < m:
            ...
            # xBall = self.SamplingUnitNBallSurface()
            ...
            in_y_range = (
                self.y_range[0] + delta <= node.y <= self.y_range[1] - delta
            )
    ...
    def SampleFreeSpaceEllipsoid(self, m, cMax, cMin, xCenter, C):
        cMaxGuess = 1.5 * cMin
        Sample = self.SampleEllipsoid(m, cMaxGuess, cMin, xCenter, C)
        return Sample
    ...
    def ExpandVertex(self, v):
        ...
        if v not in self.Tree.V_old:
            V_near = {
                w for w in self.Tree.V if self.calc_dist(w, v) <= self.Tree.r
            }

            for w in V_near:
                if (
                    (v, w) not in self.Tree.E
                    and self.g_estimated(v)
                    + self.calc_dist(v, w)
                    + self.h_estimated(w)
                    < self.g_T[self.x_goal]
