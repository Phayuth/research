...
int main(int /*argc*/, char ** /*argv*/) {
    ...
    const std::string urdf_filename =
        std::string("../rnd_torus/example-robot-data/robots/"
    ...
    GeometryData geom_data(
        geom_model); // contained the intermediate computations, like the placement
                     // of all the geometries with respect to the world frame

    // Load the reference configuration of the robots (this one should be collision
    // free)
    pinocchio::srdf::loadReferenceConfigurations(
        model,
        srdf_filename); // the reference configuratio stored in the SRDF file is
                        // called half_sitting
    ...
    const Model::ConfigVectorType q_neutral = randomConfiguration(model);
    updateGeometryPlacements(
        ...
        q_neutral); // performs a forward kinematics over the whole kinematics
                    // model + update the placement of all the geometries contained
                    // inside geom_model
