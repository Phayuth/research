...
class ConcaveHull(object):
    ...
    def __init__(self, points, k):
        if isinstance(points, np.core.ndarray):
            ...
            raise ValueError(
                "Please provide an [N,2] numpy array or a list of lists."
            )
        ...
        self.indices = np.ones(
            self.data_set.shape[0], dtype=bool
        )  # bool of a column of all 1's
    ...
    def norm_array(v):  # normalize row vectors in an array. observations are rows
        norms = np.array(
            np.sqrt(np.sum(np.square(v), axis=1)), ndmin=2
        ).transpose()
    ...
    def get_k_nearest(self, ix, k):
        ...
        distances = self.dist_pt_to_group(
            self.data_set[ixs, :], self.data_set[ix, :]
        )
    ...
    def clockwise_angles(
        self, last, ix, ixs, first
    ):  # last needs to be the index of the previous current point
        if first == 1:
            last_norm = np.array([-1, 0], ndmin=2)
        elif first == 0:
            last_norm = self.norm(
                np.subtract(self.data_set[last, :], self.data_set[ix, :])
            )  # normalized vector pointing towards previous point
        ixs_norm = self.norm_array(
            np.subtract(self.data_set[ixs, :], self.data_set[ix, :])
    ...
    def calculate(self):
        ...
        while ((current_point != first_point) or (step == 2)) and len(
        ...
        ) > 0:  # last condition counts number of ones, points in dataset
            if step == stop:
            ...
            knn = self.get_k_nearest(
                current_point, kk
            )  # knn = [3,6,2] or [0,2,7] etc indicies into the full dataset (with no points removed)
            ...
            self.indices[current_point] = (
                False  # we remove the newly found current point from the "mask" indicies so that it wont be passed to get_k_nearest (within the implicit input, self)
            )
            step += 1
